...
//   const questionPromises = chunks.map(chunk => generateQuestionsFromText(chunk));
//   const questions = await Promise.all(questionPromises);
//   // const flattenedQuestions = questions.flat();
//   // const uniqueQuestions = Array.from(new Set(flattenedQuestions.map(q => JSON.stringify(q)))).map(q => JSON.parse(q));
//   // const limitedQuestions = uniqueQuestions.slice(0, 21);
//   // return NextResponse.json({ questions: limitedQuestions });

//   try {
//     const questions = await generateQuestionsFromText(extractedText);
